...
| 模块 | 角色 |
|---|---|
| `settings_store.py` | **唯一真源。** UI（`rSettings`）读写，落盘 `~/.mino-nexus/settings.json`。含 6 个 provider 预设、邮件、Figma、role prompt override、`case_execution_use` 等业务开关 |
...
搬迁初期本模块自己读 `ai_providers.json` + env，**和 UI 写的 `settings.json` 是两套**，
...
from mino_nexus import settings_store
...
def get_ai_web_compress_ratio(provider_id: Optional[str] = None) -> float:
    """Web 截图压缩比；1.0=不压缩，默认 2.0。随 OBSERVE 下发给 Scout（协议 §4.4）。"""
