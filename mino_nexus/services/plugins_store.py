...
- Console：`plugin_policies` —— 客户端是否展示、是否启用。不存密钥。
- Studio：`user_plugin_secrets` —— 当前登录用户自己的 key。
- 执行任务时按发起用户的 user_id 取 key，不共用 settings。

能力目录在 catalog_entries，不在这个模块。
"""
...
def _policy_root() -> dict[str, Any]:
    from mino_nexus.core.database import SessionLocal, ensure_db
    from mino_nexus.models.plugin import PluginPolicy

    ensure_db()
    db = SessionLocal()
    try:
        existing = {r.plugin_id: r for r in db.query(PluginPolicy).all()}
    finally:
        db.close()
    ...
    for spec in INTEGRATION_PLUGIN_SPECS:
        ...
        out["plugins"][pid] = {
            "visible": True if row is None else bool(row.visible),
            "enabled": True if row is None else bool(row.enabled),
        }
    return out


def _persist_policy(root: dict[str, Any]) -> None:
    from mino_nexus.core.database import session_scope
    from mino_nexus.models.plugin import PluginPolicy

    plugins = root.get("plugins") if isinstance(root.get("plugins"), dict) else {}
    with session_scope() as db:
        keep = set()
        for pid, row in plugins.items():
            if not isinstance(row, dict):
                continue
            keep.add(str(pid))
            db.merge(PluginPolicy(
                plugin_id=str(pid),
                visible=row.get("visible", True) is not False,
                enabled=row.get("enabled", True) is not False,
            ))
        for row in db.query(PluginPolicy).all():
            if row.plugin_id not in keep:
                db.delete(row)
