...
from mino_nexus.device_store import remember as remember_device
from mino_nexus.log import SLog
from mino_nexus.node_store import get_node as stored_node, sanitize_id, upsert as persist_node
...
class NodeRegistry:
    ...
    def node_event(self, ev: P.NodeEvent) -> list[str]:
        """处理 NODE_EVENT。`shutting_down` 返回当时的在途 run_id，给调用方立刻中断。"""
        interrupted: list[str] = []
        with self._lock:
            node = self._nodes.get(ev.node_id)
            if node is None:
                return []
            if ev.event == "device_lost" and ev.sn in node.devices:
                if is_legacy_web_sn(ev.sn):
                    node.devices.pop(ev.sn, None)
                    if self._owner.get(ev.sn) == ev.node_id:
                        self._owner.pop(ev.sn, None)
                else:
                    for ch in node.devices[ev.sn].channels:
                        node.devices[ev.sn].channels[ch] = "disconnected"
            elif ev.event == "device_found" and ev.sn:
                if ev.sn not in node.devices:
                    node.devices[ev.sn] = P.DeviceManifest(sn=ev.sn, platform="", channels={})
                self._place_device(ev.sn, ev.node_id, device=node.devices[ev.sn], session=node)
            elif ev.event == "shutting_down":
                node.draining = True
                node.busy = False
            ...
            if node is not None:
                node.persist_snapshot()
        SLog.i(TAG, f"NODE_EVENT {ev.event} node={ev.node_id} sn={ev.sn} {ev.detail}")
        return interrupted
