"""只读 catalog_entries。kind∈{prep,do,check,generic} 为可调用能力，recovery 为恢复规则。"""
from __future__ import annotations

import threading
from typing import Any, Optional

from mino_nexus.catalog.exec_classes import CAPABILITY_KINDS, LOCAL_ORCH_IDS
    ...
    def _hydrate_row(self, row) -> None:
        ...
        if kind in CAPABILITY_KINDS:
            ...
            if not impls and row.id not in LOCAL_ORCH_IDS:
                return
            try:
                self._capabilities[row.id] = Capability(
                    id=row.id,
                    kind=kind,
        ...
        if kind == "recovery":
            try:
                self._recovery_rules[row.id] = RecoveryRule(
                    id=row.id,
