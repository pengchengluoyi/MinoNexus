...
port(nexus): 源自 MiniOrangeServer `server/services/ai/regression/schemas.py`，
**只保留跨界那三个**：`PlanEvent`、`EventResult`、`CapturedScreen`。

这三个是**跨界数据**（要经协议在两仓之间走），所以两仓各有一份、字段必须一致。
上游那个文件里 `PlanResult` / `ReplanResult` / `AgentDecision` / `CaseSpec` /
`RunReport` 等等是 LLM 的输入输出与编排结果 —— 那些只在本仓，随 `ai/` 一起搬。

`EventStatus` **不在这里定义**，从 `mino_nexus.protocol` import —— 一个定义、一处维护，
且 protocol.py 是零依赖的（裸 python3 就能跑）。

字段与上游逐字一致（含 `extra="allow"`），这样搬来的循环代码不用改。
"""
...
class PlanEvent(BaseModel):
    ...
    # ---- 以下是拆分新增，上游没有 ----
    # Scout 不读能力目录，所以执行所需的一切由本仓算好塞进 EXECUTE（CLAUDE.md §2.1）
    executor_order: list[str] = Field(default_factory=list, description="严格按此顺序尝试")
    low_level: dict[str, Any] = Field(default_factory=dict, description="抄自 catalog_entries implementations")
...
class EventResult(BaseModel):
    ...
    capability_id: str
    event_kind: str = ""
    lane: str = Field("", description="prep | do | check：服务用例哪一列。Scout 原样透传")
    status: EventStatus
    ...
    thumb: str = Field("", description="步骤缩略图 JPEG base64。由本仓的 make_thumb 填（Scout 回原图）")
...
class CapturedScreen(BaseModel):
    ...
    port(nexus): 源自 `server/services/regression/screen.py` 的同名 dataclass，
    ...
    width: int = 0
    height: int = 0
    path: str = Field("", description="Scout 本地落盘路径（Nexus 拿不到该文件，仅作排障线索）")
    error: str = ""
    elapsed_ms: int = 0
