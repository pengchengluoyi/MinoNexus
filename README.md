...
| 能力目录（`plugins/**.yaml`）唯一真源 | 不实现能力 —— 只声明"能做什么、用什么实现" |
...
执行循环在 Nexus：每一步 `OBSERVE` 取屏幕 → 本地跑 LLM 决策 → `EXECUTE` 派给 Scout。
...
| [docs/PROTOCOL.md](docs/PROTOCOL.md) | 九条消息的字段级定义（与 Scout 同一份） |
...
硬约束由脚本静态断言，CI 必跑：不含设备驱动、不 import Scout、协议契约一致。UI 契约另跑 `python scripts/check_http_contract.py`（需已安装依赖）。
